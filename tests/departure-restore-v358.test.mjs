...
assert.match(css,/\.cph-now-icon,[\s\S]*?\.cph-aside-icon\{[\s\S]*?display:none!important/,'les anciennes icônes d’action doivent être supprimées visuellement');
...
assert.match(css,/\.cph-section-title>span:first-child\{[\s\S]*?right:calc\(100% \+ 7px\)!important/,'l’icône des en-têtes doit être placée juste à gauche sans déplacer le texte');
...
assert.match(sw,/copenhague-v358-static-v55/,'le cache statique doit être renouvelé pour la v367');
assert.match(sw,/\/interaction-layout-v358\.css\?v=358/,'la feuille v358 doit être précachée');

console.log(JSON.stringify({ok:true,departure:'never-painted',actions:'iconless-fixed-first-paint-v367',editor:'optically-centered',warnings:'compact-centered',sections:'true-center',openTitle:'visit-duration-axis',photos:'missing-and-broken-local-fallback',cascade:'clicked-point-first'},null,2));
